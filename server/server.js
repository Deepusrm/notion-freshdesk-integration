  ...
  onConversationCreateHandler: async function (args) {
    ...
    if (noteData.private == true) {
      ...
      try {
        ...
        console.log(ticketConversations[conversationId]);

         //ticketConversations.push({[conversationId]:blockIds});
        ...
        if (error.message === "The setIf conditional request failed") {
          ...
          const updatedTicketDetails = {...notion_page_id};
          ...
          const results = JSON.parse(responses.response);
          const filteredResults = results["results"].filter((element)=> element["type"]!= "divider");
          ...
          //ticketDetails["conversations"].push({[conversationId]:blockIds});
          // notion_page_id["ticketDetails"]["conversations"][conversationId] = blockIds;
          let conversation = "ticket.conversations."+conversationId;
          const response = await $db.update(ticketKey,"set",{[conversation]:blockIds},{setIf:"exist"});
